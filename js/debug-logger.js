...
export async function logShotEvent(eventData) {
  ...
  try {
    ...
    const logEntry = {
      ...
      keeperStats:
        APP.mode === 'defense' && APP.activeKeeper
          ? {
    ...
    await db.collection('users').doc(user.uid).collection('debug_logs').add(logEntry);
...
export async function logError(error, context = {}) {
  try {
    ...
    await db.collection('users').doc(user.uid).collection('errors').add(errorEntry);

  ...
  if (APP.mode === 'defense') {
    if (!APP.activeKeeper) {
      ...
      if (firstKeeper) {
        ...
        alert(
          'Du må velge en aktiv keeper før du kan registrere forsvar!\n\nGå til oppsettet og merk minst én spiller som keeper.'
        );
        return false;
      }
    }
...
export function selectShotPosition(
  position,
  closeModal,
...
export function renderShotPopupContent() {
  ...
  const needsShotPosition =
    isDetailedMode && APP.selectedShooter && APP.selectedAttackType && !APP.selectedShotPosition;
  const needsAssist =
    isDetailedMode &&
    APP.mode === 'attack' &&
    APP.selectedResult === 'mål' &&
    APP.selectedShooter &&
    APP.selectedAttackType &&
    APP.selectedShotPosition &&
    !APP.selectedAssist;
  ...
  if (
    ...
    if (APP.selectedAttackType) {
      parts.push(
        `Angrep: ${APP.selectedAttackType === 'etablert' ? '🏃 Etablert' : '⚡ Kontring'}`
      );
    }
  ...
  return `
        ...
        `
            ...
            <div>
                ...
                <div class="player-grid">
                    ${APP.players
                      .filter((p) => p.id !== APP.selectedShooter)
                      .map(
                        (player) => `
...
export function updateGoalVisualization() {
  ...
  const shots = APP.events.filter(
    (e) => e.mode === APP.mode && (e.player || e.opponent) && e.zone === 'goal'
  );

  const outsideShots = APP.events.filter(
    (e) => e.mode === APP.mode && (e.player || e.opponent) && e.zone === 'outside'
  );

...
export function getPlayerStats(playerId, half = null) {
  ...
  return PERFORMANCE.getCachedStats(`player-${playerId}-${half}`, () => {
    ...
    const playerEvents = events.filter(
      (e) => e.player?.id === playerId && (half === null || e.half === half) && e.mode === 'attack'
    );
    return {
      ...
      technical: events.filter(
        (e) =>
          e.player?.id === playerId && e.mode === 'technical' && (half === null || e.half === half)
      ).length,
    };
  });
}
...
export function getOpponentStats(opponentId, half = null) {
  ...
  return PERFORMANCE.getCachedStats(`opponent-${opponentId}-${half}`, () => {
    ...
    const opponentEvents = events.filter(
      (e) =>
        e.opponent?.id === opponentId && (half === null || e.half === half) && e.mode === 'defense'
    );
    return {

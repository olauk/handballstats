...
export function showKeeperShotDetails(keeperId) {
  ...
  const events = getCurrentEvents();
  const keeperShots = events.filter((e) => e.keeper?.id === keeperId && e.mode === 'defense');
...
export function renderShotDetailsPopupContent() {
  ...
  const goalShotMarkers = goalShots
    .map((shot) => {
      const playerNumber = isKeeper
  ...
  const outsideShotMarkers = outsideShots
    .map((shot) => {
      const playerNumber = isKeeper
        ? shot.opponent?.number
        : isOpponent
